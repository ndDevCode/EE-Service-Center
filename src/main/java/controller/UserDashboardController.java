...
public class UserDashboardController {
    ...
    private void loadViewReport(StaffDto staffDto) {
        try {
            FXMLLoader loader = new FXMLLoader(Objects.requireNonNull
                    (getClass().getResource("/view/UserReport.fxml")));
            Parent root = loader.load();
            UserReportController userReportController = loader.getController();
            userReportController.initLoggedUser(staffDto);
            paneContent.getChildren().setAll((Node) root);
        } catch (IOException e) {
            e.printStackTrace();
        }
